...
while(1)
{
    ...
    while(cin.fail() || ((choose != 1) && (choose != 2) && (choose != 3) &&(choose !=4) && (choose !=5) && (choose!=6)))    //validation process
    {
        cin.clear();
        cin.ignore(100000000, '\n');
        cout << endl << "invalid! input proper number" << endl << endl;
    ...
    if(choose == 2)
    {
        ...
        pSpace = pQueue->GetFront();
        pQueue->GetFront()->SetEntrance(pSpace);
    }
    #endif
    #if 1
    #if 0
    ...
    {
        pSpace = pQueue->GetCurrent()
        pSpace = pQueue->GetCurrent()->GetNext();
        cout << "now the colonel is at " << PQueue->GetCurrent()->GetSpace()->GetName()<< endl;
    }
    else if(choose == 5 )
    {
        pSpace = pQueue->GetCurrent() 
        pSpace = pQueue->GetCurrent()->GetPrev();
        cout << "now the colonel is at " << PQueue->GetCurrent()->GetSpace()->GetName()<< endl;
