...
/********************************************************************* 
** Function : print
** Description : printing the value of the stack
** Parameters : none
** Pre-Conditions : stack -like structure.
** Post-Conditions : removing a chracter on the top of the stack like structure and return the character being removed.
*********************************************************************/
void DynCharStack::PrintStack()
{
    int stackOrder = 0;
    StackNode* temp = top;
    if(top == NULL)
    ...
    while(top != NULL)
    {
        string name = top->pLoser->GetName();
        int creature_st = top->pLoser->GetStrength();
        top = top->next;
        stackOrder++;
        cout << "#" << stackOrder << " = " << name << " : strength : " << creature_st << endl;
    }
...
** Description : deleting the stack
** Parameters : none
** Pre-Conditions : stack -like structure.
** Post-Conditions : deleting the exsiting stack.
*********************************************************************/
void DynCharStack::DeleteStack()
{
    while(top != NULL)
    {
        StackNode* temp = top;
