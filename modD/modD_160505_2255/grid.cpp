            ...
            if(grid[i][j] == 'A')
            {
               ...
               if(move ==0)
               {
                   val = ValidMoveCheck(newX, newY);
                      if((val == 1) || (val == 2))  //val is not possible to move. 
                        {
                            x = oldX;
                            y = oldY;
                			tempGrid[x][y] = 'T';    
...
printf("[%s:%d]\n",__FILE__,__LINE__);
               ...
               {
                   val = ValidMoveCheck(newX, newY);
                      if(val == 1) //val is not possible to move. 
                        {
                            x = oldX;
                            y = oldY;
                			tempGrid[x][y] = 'S';    
						}
                      else if(val == 2) //val is not possible to move. 
                        {
                            x = newX;
                            y = newY;
printf("[%s:%d]\n",__FILE__,__LINE__);
                            pCritter[x][y] = pCritter[oldX][oldY]->Eat();
printf("[%s:%d]\n",__FILE__,__LINE__);
printf("[%s:%d]\n",__FILE__,__LINE__);
...
printf("[%s:%d]\n",__FILE__,__LINE__);
printf("[%s:%d]\n",__FILE__,__LINE__);
             //DoodleBug* pDoodle = static_cast<DoodleBug*>(pCritter[x][y]);
printf("[%s:%d]\n",__FILE__,__LINE__);
               pCritter[x][y] =  pDoodle->Starve();
printf("[%s:%d]\n",__FILE__,__LINE__);
                if(pCritter[x][y] == NULL)
                {
printf("[%s:%d]\n",__FILE__,__LINE__);
...
//                cout << "pDoodle value " << pDoodle->GetStarveCount() << endl;
printf("[%s:%d]\n",__FILE__,__LINE__);

                 int starveCount = pDoodle->GetStarveCount();
printf("[%s:%d]\n",__FILE__,__LINE__);
                   starveCount++;
printf("[%s:%d]\n",__FILE__,__LINE__);
                   pDoodle->SetStarveCount(starveCount);
printf("[%s:%d]\n",__FILE__,__LINE__);
               // cout << endl << endl <<"starve count is " << starveCount << endl << endl;
               // if( starveCount > 3 )
               // {
                  pCritter[x][y] =  pDoodle->Starve();
                if(pCritter[x][y] == NULL)
...
printf("[%s:%d]\n",__FILE__,__LINE__);
                      pDoodle->SetStarveCount(starveCount++);
printf("[%s:%d]\n",__FILE__,__LINE__);
                      cout << pDoodle->GetStarveCount() << endl;
printf("[%s:%d]\n",__FILE__,__LINE__);
...
printf("[%s:%d]\n",__FILE__,__LINE__);
	    for(int a = 0; a < rows; a++)
        {
            ...
            {
printf("[%s:%d]\n",__FILE__,__LINE__);
             DoodleBug* pDoodle = static_cast<DoodleBug*>(pCritter[a][b]);
printf("[%s:%d]\n",__FILE__,__LINE__);
               pCritter[a][b] =  pDoodle->Starve();
printf("[%s:%d]\n",__FILE__,__LINE__);
