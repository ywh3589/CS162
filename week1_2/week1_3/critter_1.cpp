...
void Critter::MoveCritter(Grid *pGrid)
{
    ...
    char **pStatus = pGrid->UpdateLocation(x, y); //StartPoint only makes new position does not set the position to older position.
...
void ValidMoveCheck(Grid *pGrid)
{
       ...
       int rows_size = pGrid->GetRows();
       int column_size = pGrid->GetColumns();
