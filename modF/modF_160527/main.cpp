...
//test
    ...
    for(int i = 0 ; i < 4*timeStamp; i++)
    {
     ...
     int westDepart =  s1.nrOfDepartAt2Light();
        ...
        /*Cars on east bound lane are moving*/
        
        for(int i = 0; i < eastArrive; i++)
        s1.GetQueue(0)->add(timeStamp);
        for(int i = 0; i < eastDepart; i++)
        s1.GetQueue(0)->remove();
        
        
        /*Cars on west bound lane are moving*/
        
        for(int i = 0; i < westArrive; i++)
        s1.GetQueue(1)->add(timeStamp);
        for(int i = 0; i < westDepart; i++)
        s1.GetQueue(1)->remove();
